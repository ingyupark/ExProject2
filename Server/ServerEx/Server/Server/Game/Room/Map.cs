...
namespace Server.Game
{
	...
	public class Map
	{
		...
		public Vector2Int _portalpos;
		public Vector2Int _portal2pos;
		...
		public bool potalGo(Vector2Int cellPos, bool checkObjects = true)
		{
			// DestPosㄹㅡㄹ ㄴㅓㅎㅇㅡㅁㅕㄴ...d포탈고가 true가 되게끔..
			if (cellPos.x < MinX || cellPos.x > MaxX)
				return false;
			if (cellPos.y < MinY || cellPos.y > MaxY)
				return false;
		...
		public bool potal2Go(Vector2Int cellPos, bool checkObjects = true)
		{
			// DestPosㄹㅡㄹ ㄴㅓㅎㅇㅡㅁㅕㄴ...d포탈고가 true가 되게끔..
			if (cellPos.x < MinX || cellPos.x > MaxX)
				return false;
		...
		public bool ApplyMove(GameObject gameObject, Vector2Int dest, bool checkObjects = true, bool collision = true)
		{
			...
			// 성공햇다..!
			if (potalGo(dest) && gameObject.ObjectType == GameObjectType.Player)
			{
			...
			else if (potal2Go(dest) && gameObject.ObjectType == GameObjectType.Player)
			{
		...
		public void LoadMap(int mapId, string pathPrefix = "../../../../../Common/MapData")
		{
			...
			_portal2 = new bool[yCount, xCount];
			_portalpos = new Vector2Int();
			_portal2pos = new Vector2Int();
			...
			for (int y = 0; y < yCount; y++)
			{
				...
				for (int x = 0; x < xCount; x++)
				{
					...
					_portal2[y, x] = (line[x] == '3' ? true : false);
					...
					if (line[x] == '2')
					{
						...
						Console.WriteLine(_portalpos.y);
						Console.WriteLine(_portalpos.x);
					}
					...
					if (line[x] == '3')
					{
						_portal2pos.y = y;
						_portal2pos.x = x;
						//MaxY - cell.y, cell.x - MinX
						Console.WriteLine(_portal2pos.y);
						Console.WriteLine(_portal2pos.x);
					}
				}
			}
		}
